...
def Show():
    try:
        ...
        n = 1
        for i in res :
            Lb1.insert(n,"{}  -  {}  --  {}  --  {}  ---  {} -- {:.2f}".format(n,i[0],i[1],i[2],i[3],i[3]/60))
            n += 1
        ...
        Lb1.pack(fill="x")
        lable2 = Label(show,text="{} minutes or {:.2f} hours".format(Sum[0],(Sum[0])/60))
        lable2.pack(fill="x")
    except Exception as e:
...
def PRINT():
    try:
        ...
        n = 1
        for i in res :
            f.write(f"{n},{i[0]},{i[1]},{i[2]},{i[3]},{i[4]},{i[5]},{i[6]},{i[7]},{i[7]/60:.2f}\n")
            # f.write("{}\t,{}\n".format(n,i))
            n += 1
...
def Start():
    ...
    c.execute("SELECT SUM FROM kartable WHERE month = {} and year = {} ORDER BY ID DESC LIMIT 1".format(date_s[1],date_s[0]))
    ...
    try:
        var.set("{} miniutes or {:.2f} hours".format(SUM[0],(SUM[0])/60))
    except Exception as e:
        print("seting var in line 138 has eror --> ",e,"so we set 0 in it.(0 miniutes or 0 hours)")
        var.set("0 miniutes or 0 hours")
    status.pack(pady=7)
    

    btn_login = ttk.Button(win, text= 'Sign in', command = signin)
    btn_login.pack(pady=7)
    btn_signout = ttk.Button(win, text= 'Sign out', command = signout)
